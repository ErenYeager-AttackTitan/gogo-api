...
const BASE_URL3 = 'https://anitaku.bz/home.html';
...
export const scrapeAnimeDetails = async ({ id }) => {
  try {
    ...
    const animePageTest = await axios.get(`https://anitaku.bz/category/${id}`);
...
export const scrapeWatchAnime = async ({ id }) => {
  try {
    ...
    const WatchAnime = await axios.get(`https://anitaku.bz/${id}`);
